...
func (*DevicePoster) scanValues(columns []string) ([]interface{}, error) {
	...
	for i := range columns {
		...
		default:
			return nil, fmt.Errorf("unexpected column %q for type DevicePoster", columns[i])
		}
	}
	return values, nil
}
...
func (dp *DevicePoster) assignValues(columns []string, values []interface{}) error {
...
func (dp *DevicePoster) Update() *DevicePosterUpdateOne {
	return (&DevicePosterClient{config: dp.config}).UpdateOne(dp)
}
...
func (dp *DevicePoster) Unwrap() *DevicePoster {
	...
	if !ok {
		panic("ent: DevicePoster is not a transactional entity")
	}
...
func (dp DevicePosters) config(cfg config) {
	for _i := range dp {
		dp[_i].config = cfg
	}
}
