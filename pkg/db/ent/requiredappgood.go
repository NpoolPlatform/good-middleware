...
func (*RequiredAppGood) scanValues(columns []string) ([]interface{}, error) {
	...
	for i := range columns {
		...
		default:
			return nil, fmt.Errorf("unexpected column %q for type RequiredAppGood", columns[i])
		}
	}
	return values, nil
}
...
func (rag *RequiredAppGood) assignValues(columns []string, values []interface{}) error {
...
func (rag *RequiredAppGood) Update() *RequiredAppGoodUpdateOne {
	return (&RequiredAppGoodClient{config: rag.config}).UpdateOne(rag)
}
...
func (rag *RequiredAppGood) Unwrap() *RequiredAppGood {
	...
	if !ok {
		panic("ent: RequiredAppGood is not a transactional entity")
	}
...
func (rag RequiredAppGoods) config(cfg config) {
	for _i := range rag {
		rag[_i].config = cfg
	}
}
