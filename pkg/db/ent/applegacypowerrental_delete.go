...
func (alprd *AppLegacyPowerRentalDelete) Exec(ctx context.Context) (int, error) {
	...
	if len(alprd.hooks) == 0 {
		affected, err = alprd.sqlExec(ctx)
	} else {
		var mut Mutator = MutateFunc(func(ctx context.Context, m Mutation) (Value, error) {
			...
			alprd.mutation = mutation
			affected, err = alprd.sqlExec(ctx)
			mutation.done = true
			return affected, err
		})
		for i := len(alprd.hooks) - 1; i >= 0; i-- {
			if alprd.hooks[i] == nil {
				return 0, fmt.Errorf("ent: uninitialized hook (forgotten import ent/runtime?)")
			}
			mut = alprd.hooks[i](mut)
		}
		if _, err := mut.Mutate(ctx, alprd.mutation); err != nil {
			return 0, err
		}
	}
	return affected, err
