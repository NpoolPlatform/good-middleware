...
func (apru *AppPowerRentalUpdate) Save(ctx context.Context) (int, error) {
	...
	if err := apru.defaults(); err != nil {
		return 0, err
	}
	if len(apru.hooks) == 0 {
		affected, err = apru.sqlSave(ctx)
	} else {
		var mut Mutator = MutateFunc(func(ctx context.Context, m Mutation) (Value, error) {
			...
			apru.mutation = mutation
			affected, err = apru.sqlSave(ctx)
			mutation.done = true
			return affected, err
		})
		for i := len(apru.hooks) - 1; i >= 0; i-- {
			if apru.hooks[i] == nil {
				return 0, fmt.Errorf("ent: uninitialized hook (forgotten import ent/runtime?)")
			}
			mut = apru.hooks[i](mut)
		}
		if _, err := mut.Mutate(ctx, apru.mutation); err != nil {
			return 0, err
		}
	}
	return affected, err
...
func (apru *AppPowerRentalUpdate) defaults() error {
	if _, ok := apru.mutation.UpdatedAt(); !ok {
		if apppowerrental.UpdateDefaultUpdatedAt == nil {
			return fmt.Errorf("ent: uninitialized apppowerrental.UpdateDefaultUpdatedAt (forgotten import ent/runtime?)")
		}
...
func (apruo *AppPowerRentalUpdateOne) Save(ctx context.Context) (*AppPowerRental, error) {
	...
	if err := apruo.defaults(); err != nil {
		return nil, err
	}
	if len(apruo.hooks) == 0 {
		node, err = apruo.sqlSave(ctx)
	} else {
		var mut Mutator = MutateFunc(func(ctx context.Context, m Mutation) (Value, error) {
			mutation, ok := m.(*AppPowerRentalMutation)
			if !ok {
			...
			apruo.mutation = mutation
			node, err = apruo.sqlSave(ctx)
			mutation.done = true
			return node, err
		})
		for i := len(apruo.hooks) - 1; i >= 0; i-- {
			if apruo.hooks[i] == nil {
				return nil, fmt.Errorf("ent: uninitialized hook (forgotten import ent/runtime?)")
			}
			mut = apruo.hooks[i](mut)
		}
		v, err := mut.Mutate(ctx, apruo.mutation)
		if err != nil {
			return nil, err
		}
...
// defaults sets the default values of the builder before save.
func (apruo *AppPowerRentalUpdateOne) defaults() error {
...
func (apruo *AppPowerRentalUpdateOne) sqlSave(ctx context.Context) (_node *AppPowerRental, err error) {
	...
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`ent: missing "AppPowerRental.id" for update`)}
	}
