...
func (agpu *AppGoodPosterUpdate) defaults() error {
	if _, ok := agpu.mutation.UpdatedAt(); !ok {
		if appgoodposter.UpdateDefaultUpdatedAt == nil {
			return fmt.Errorf("ent: uninitialized appgoodposter.UpdateDefaultUpdatedAt (forgotten import ent/runtime?)")
		}
...
func (agpu *AppGoodPosterUpdate) sqlSave(ctx context.Context) (n int, err error) {
	...
	_spec.Modifiers = agpu.modifiers
...
func (agpuo *AppGoodPosterUpdateOne) Save(ctx context.Context) (*AppGoodPoster, error) {
	...
	if len(agpuo.hooks) == 0 {
		...
		v, err := mut.Mutate(ctx, agpuo.mutation)
		if err != nil {
			return nil, err
		}
...
// defaults sets the default values of the builder before save.
func (agpuo *AppGoodPosterUpdateOne) defaults() error {
...
func (agpuo *AppGoodPosterUpdateOne) sqlSave(ctx context.Context) (_node *AppGoodPoster, err error) {
	...
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`ent: missing "AppGoodPoster.id" for update`)}
	}
	...
	_spec.Modifiers = agpuo.modifiers
