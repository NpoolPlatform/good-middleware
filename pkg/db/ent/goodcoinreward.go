...
func (*GoodCoinReward) scanValues(columns []string) ([]interface{}, error) {
	...
	for i := range columns {
		...
		default:
			return nil, fmt.Errorf("unexpected column %q for type GoodCoinReward", columns[i])
		}
	}
	return values, nil
}
...
func (gcr *GoodCoinReward) assignValues(columns []string, values []interface{}) error {
...
func (gcr *GoodCoinReward) Update() *GoodCoinRewardUpdateOne {
	return (&GoodCoinRewardClient{config: gcr.config}).UpdateOne(gcr)
}
...
func (gcr *GoodCoinReward) Unwrap() *GoodCoinReward {
	...
	if !ok {
		panic("ent: GoodCoinReward is not a transactional entity")
	}
...
func (gcr GoodCoinRewards) config(cfg config) {
	for _i := range gcr {
		gcr[_i].config = cfg
	}
}
