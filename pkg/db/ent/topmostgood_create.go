...
func (tmgc *TopMostGoodCreate) Save(ctx context.Context) (*TopMostGood, error) {
	...
	if len(tmgc.hooks) == 0 {
		...
		var mut Mutator = MutateFunc(func(ctx context.Context, m Mutation) (Value, error) {
			...
			tmgc.mutation = mutation
			if node, err = tmgc.sqlSave(ctx); err != nil {
				return nil, err
			}
			mutation.id = &node.ID
			mutation.done = true
		...
		for i := len(tmgc.hooks) - 1; i >= 0; i-- {
			if tmgc.hooks[i] == nil {
				return nil, fmt.Errorf("ent: uninitialized hook (forgotten import ent/runtime?)")
			}
			mut = tmgc.hooks[i](mut)
		}
		v, err := mut.Mutate(ctx, tmgc.mutation)
		if err != nil {
			return nil, err
		}
		nv, ok := v.(*TopMostGood)
...
func (tmgc *TopMostGoodCreate) defaults() error {
	if _, ok := tmgc.mutation.CreatedAt(); !ok {
		if topmostgood.DefaultCreatedAt == nil {
			return fmt.Errorf("ent: uninitialized topmostgood.DefaultCreatedAt (forgotten import ent/runtime?)")
		}
		v := topmostgood.DefaultCreatedAt()
		tmgc.mutation.SetCreatedAt(v)
	}
	if _, ok := tmgc.mutation.UpdatedAt(); !ok {
		if topmostgood.DefaultUpdatedAt == nil {
			return fmt.Errorf("ent: uninitialized topmostgood.DefaultUpdatedAt (forgotten import ent/runtime?)")
		}
		v := topmostgood.DefaultUpdatedAt()
		tmgc.mutation.SetUpdatedAt(v)
	}
	if _, ok := tmgc.mutation.DeletedAt(); !ok {
		if topmostgood.DefaultDeletedAt == nil {
			return fmt.Errorf("ent: uninitialized topmostgood.DefaultDeletedAt (forgotten import ent/runtime?)")
		}
...
func (tmgc *TopMostGoodCreate) check() error {
	if _, ok := tmgc.mutation.CreatedAt(); !ok {
		return &ValidationError{Name: "created_at", err: errors.New(`ent: missing required field "TopMostGood.created_at"`)}
	}
	if _, ok := tmgc.mutation.UpdatedAt(); !ok {
		return &ValidationError{Name: "updated_at", err: errors.New(`ent: missing required field "TopMostGood.updated_at"`)}
	}
	if _, ok := tmgc.mutation.DeletedAt(); !ok {
...
func (u *TopMostGoodUpsertOne) Exec(ctx context.Context) error {
	if len(u.create.conflict) == 0 {
		return errors.New("ent: missing options for TopMostGoodCreate.OnConflict")
	}
...
func (u *TopMostGoodUpsertBulk) Exec(ctx context.Context) error {
	for i, b := range u.create.builders {
		if len(b.conflict) != 0 {
			return fmt.Errorf("ent: OnConflict was set for builder %d. Set it on the TopMostGoodCreateBulk instead", i)
		}
	}
	if len(u.create.conflict) == 0 {
		return errors.New("ent: missing options for TopMostGoodCreateBulk.OnConflict")
	}
	return u.create.Exec(ctx)
}
