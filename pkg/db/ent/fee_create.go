...
func (fc *FeeCreate) defaults() error {
	...
	if _, ok := fc.mutation.EntID(); !ok {
		if fee.DefaultEntID == nil {
			return fmt.Errorf("ent: uninitialized fee.DefaultEntID (forgotten import ent/runtime?)")
		}
		v := fee.DefaultEntID()
		fc.mutation.SetEntID(v)
	}
	if _, ok := fc.mutation.GoodID(); !ok {
		if fee.DefaultGoodID == nil {
			return fmt.Errorf("ent: uninitialized fee.DefaultGoodID (forgotten import ent/runtime?)")
		}
		v := fee.DefaultGoodID()
...
func (fc *FeeCreate) check() error {
	...
	if _, ok := fc.mutation.EntID(); !ok {
		return &ValidationError{Name: "ent_id", err: errors.New(`ent: missing required field "Fee.ent_id"`)}
	}
	return nil
}
