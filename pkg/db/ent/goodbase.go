...
func (*GoodBase) scanValues(columns []string) ([]interface{}, error) {
	...
	for i := range columns {
		...
		default:
			return nil, fmt.Errorf("unexpected column %q for type GoodBase", columns[i])
		}
	}
	return values, nil
}
...
func (gb *GoodBase) assignValues(columns []string, values []interface{}) error {
...
func (gb *GoodBase) Update() *GoodBaseUpdateOne {
	return (&GoodBaseClient{config: gb.config}).UpdateOne(gb)
}
...
func (gb *GoodBase) Unwrap() *GoodBase {
	...
	if !ok {
		panic("ent: GoodBase is not a transactional entity")
	}
...
func (gb GoodBases) config(cfg config) {
	for _i := range gb {
		gb[_i].config = cfg
	}
}
