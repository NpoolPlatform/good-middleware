...
func (vbc *VendorBrandCreate) Save(ctx context.Context) (*VendorBrand, error) {
	...
	if len(vbc.hooks) == 0 {
		...
		var mut Mutator = MutateFunc(func(ctx context.Context, m Mutation) (Value, error) {
			...
			if node, err = vbc.sqlSave(ctx); err != nil {
				return nil, err
			}
			mutation.id = &node.ID
			mutation.done = true
		...
		v, err := mut.Mutate(ctx, vbc.mutation)
		if err != nil {
			return nil, err
		}
		nv, ok := v.(*VendorBrand)
...
func (vbc *VendorBrandCreate) defaults() error {
	if _, ok := vbc.mutation.CreatedAt(); !ok {
		if vendorbrand.DefaultCreatedAt == nil {
			return fmt.Errorf("ent: uninitialized vendorbrand.DefaultCreatedAt (forgotten import ent/runtime?)")
		}
		v := vendorbrand.DefaultCreatedAt()
		vbc.mutation.SetCreatedAt(v)
	}
	if _, ok := vbc.mutation.UpdatedAt(); !ok {
		if vendorbrand.DefaultUpdatedAt == nil {
			return fmt.Errorf("ent: uninitialized vendorbrand.DefaultUpdatedAt (forgotten import ent/runtime?)")
		}
		v := vendorbrand.DefaultUpdatedAt()
		vbc.mutation.SetUpdatedAt(v)
	}
	if _, ok := vbc.mutation.DeletedAt(); !ok {
		if vendorbrand.DefaultDeletedAt == nil {
			return fmt.Errorf("ent: uninitialized vendorbrand.DefaultDeletedAt (forgotten import ent/runtime?)")
		}
...
func (vbc *VendorBrandCreate) check() error {
	if _, ok := vbc.mutation.CreatedAt(); !ok {
		return &ValidationError{Name: "created_at", err: errors.New(`ent: missing required field "VendorBrand.created_at"`)}
	}
	if _, ok := vbc.mutation.UpdatedAt(); !ok {
		return &ValidationError{Name: "updated_at", err: errors.New(`ent: missing required field "VendorBrand.updated_at"`)}
	}
	if _, ok := vbc.mutation.DeletedAt(); !ok {
...
func (u *VendorBrandUpsertOne) Exec(ctx context.Context) error {
	if len(u.create.conflict) == 0 {
		return errors.New("ent: missing options for VendorBrandCreate.OnConflict")
	}
...
func (u *VendorBrandUpsertBulk) Exec(ctx context.Context) error {
	for i, b := range u.create.builders {
		if len(b.conflict) != 0 {
			return fmt.Errorf("ent: OnConflict was set for builder %d. Set it on the VendorBrandCreateBulk instead", i)
		}
	}
	if len(u.create.conflict) == 0 {
		return errors.New("ent: missing options for VendorBrandCreateBulk.OnConflict")
	}
	return u.create.Exec(ctx)
}
