...
func (prc *PowerRentalCreate) defaults() error {
	...
	if _, ok := prc.mutation.GoodID(); !ok {
		if powerrental.DefaultGoodID == nil {
			return fmt.Errorf("ent: uninitialized powerrental.DefaultGoodID (forgotten import ent/runtime?)")
		}
		v := powerrental.DefaultGoodID()
		prc.mutation.SetGoodID(v)
	}
	if _, ok := prc.mutation.DeviceTypeID(); !ok {
		if powerrental.DefaultDeviceTypeID == nil {
			return fmt.Errorf("ent: uninitialized powerrental.DefaultDeviceTypeID (forgotten import ent/runtime?)")
		}
		v := powerrental.DefaultDeviceTypeID()
		prc.mutation.SetDeviceTypeID(v)
	}
	if _, ok := prc.mutation.VendorLocationID(); !ok {
		if powerrental.DefaultVendorLocationID == nil {
			return fmt.Errorf("ent: uninitialized powerrental.DefaultVendorLocationID (forgotten import ent/runtime?)")
		}
