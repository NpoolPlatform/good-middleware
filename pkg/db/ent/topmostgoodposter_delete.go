...
func (tmgpd *TopMostGoodPosterDelete) Exec(ctx context.Context) (int, error) {
	...
	if len(tmgpd.hooks) == 0 {
		affected, err = tmgpd.sqlExec(ctx)
	} else {
		var mut Mutator = MutateFunc(func(ctx context.Context, m Mutation) (Value, error) {
			...
			tmgpd.mutation = mutation
			affected, err = tmgpd.sqlExec(ctx)
			mutation.done = true
			return affected, err
		})
		for i := len(tmgpd.hooks) - 1; i >= 0; i-- {
			if tmgpd.hooks[i] == nil {
				return 0, fmt.Errorf("ent: uninitialized hook (forgotten import ent/runtime?)")
			}
			mut = tmgpd.hooks[i](mut)
		}
		if _, err := mut.Mutate(ctx, tmgpd.mutation); err != nil {
			return 0, err
		}
	}
	return affected, err
