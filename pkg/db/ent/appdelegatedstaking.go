...
func (*AppDelegatedStaking) scanValues(columns []string) ([]interface{}, error) {
	...
	for i := range columns {
		...
		default:
			return nil, fmt.Errorf("unexpected column %q for type AppDelegatedStaking", columns[i])
		}
	}
	return values, nil
}
...
func (ads *AppDelegatedStaking) assignValues(columns []string, values []interface{}) error {
...
func (ads *AppDelegatedStaking) Update() *AppDelegatedStakingUpdateOne {
	return (&AppDelegatedStakingClient{config: ads.config}).UpdateOne(ads)
}
...
func (ads *AppDelegatedStaking) Unwrap() *AppDelegatedStaking {
	...
	if !ok {
		panic("ent: AppDelegatedStaking is not a transactional entity")
	}
...
func (ads AppDelegatedStakings) config(cfg config) {
	for _i := range ads {
		ads[_i].config = cfg
	}
}
