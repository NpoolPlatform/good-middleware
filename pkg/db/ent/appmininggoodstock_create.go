...
func (amgsc *AppMiningGoodStockCreate) Save(ctx context.Context) (*AppMiningGoodStock, error) {
	...
	if err := amgsc.defaults(); err != nil {
		return nil, err
	}
	if len(amgsc.hooks) == 0 {
		if err = amgsc.check(); err != nil {
			return nil, err
		}
		node, err = amgsc.sqlSave(ctx)
	} else {
		var mut Mutator = MutateFunc(func(ctx context.Context, m Mutation) (Value, error) {
			...
			amgsc.mutation = mutation
			if node, err = amgsc.sqlSave(ctx); err != nil {
				return nil, err
			}
			mutation.id = &node.ID
			mutation.done = true
		...
		for i := len(amgsc.hooks) - 1; i >= 0; i-- {
			if amgsc.hooks[i] == nil {
				return nil, fmt.Errorf("ent: uninitialized hook (forgotten import ent/runtime?)")
			}
			mut = amgsc.hooks[i](mut)
		}
		v, err := mut.Mutate(ctx, amgsc.mutation)
		if err != nil {
			return nil, err
		}
...
func (amgsc *AppMiningGoodStockCreate) defaults() error {
	if _, ok := amgsc.mutation.CreatedAt(); !ok {
		if appmininggoodstock.DefaultCreatedAt == nil {
			return fmt.Errorf("ent: uninitialized appmininggoodstock.DefaultCreatedAt (forgotten import ent/runtime?)")
		}
		v := appmininggoodstock.DefaultCreatedAt()
		amgsc.mutation.SetCreatedAt(v)
	}
	if _, ok := amgsc.mutation.UpdatedAt(); !ok {
		if appmininggoodstock.DefaultUpdatedAt == nil {
			return fmt.Errorf("ent: uninitialized appmininggoodstock.DefaultUpdatedAt (forgotten import ent/runtime?)")
		}
		v := appmininggoodstock.DefaultUpdatedAt()
		amgsc.mutation.SetUpdatedAt(v)
	}
	if _, ok := amgsc.mutation.DeletedAt(); !ok {
		if appmininggoodstock.DefaultDeletedAt == nil {
			return fmt.Errorf("ent: uninitialized appmininggoodstock.DefaultDeletedAt (forgotten import ent/runtime?)")
		}
		v := appmininggoodstock.DefaultDeletedAt()
		amgsc.mutation.SetDeletedAt(v)
	}
	if _, ok := amgsc.mutation.EntID(); !ok {
		if appmininggoodstock.DefaultEntID == nil {
			return fmt.Errorf("ent: uninitialized appmininggoodstock.DefaultEntID (forgotten import ent/runtime?)")
		}
...
func (amgsc *AppMiningGoodStockCreate) check() error {
	if _, ok := amgsc.mutation.CreatedAt(); !ok {
		return &ValidationError{Name: "created_at", err: errors.New(`ent: missing required field "AppMiningGoodStock.created_at"`)}
	}
	if _, ok := amgsc.mutation.UpdatedAt(); !ok {
		return &ValidationError{Name: "updated_at", err: errors.New(`ent: missing required field "AppMiningGoodStock.updated_at"`)}
	}
	if _, ok := amgsc.mutation.DeletedAt(); !ok {
		return &ValidationError{Name: "deleted_at", err: errors.New(`ent: missing required field "AppMiningGoodStock.deleted_at"`)}
	}
	if _, ok := amgsc.mutation.EntID(); !ok {
		return &ValidationError{Name: "ent_id", err: errors.New(`ent: missing required field "AppMiningGoodStock.ent_id"`)}
	}
	return nil
}
...
func (u *AppMiningGoodStockUpsertOne) Exec(ctx context.Context) error {
	if len(u.create.conflict) == 0 {
		return errors.New("ent: missing options for AppMiningGoodStockCreate.OnConflict")
	}
...
func (u *AppMiningGoodStockUpsertBulk) Exec(ctx context.Context) error {
	for i, b := range u.create.builders {
		if len(b.conflict) != 0 {
			return fmt.Errorf("ent: OnConflict was set for builder %d. Set it on the AppMiningGoodStockCreateBulk instead", i)
		}
	}
	if len(u.create.conflict) == 0 {
		return errors.New("ent: missing options for AppMiningGoodStockCreateBulk.OnConflict")
	}
	return u.create.Exec(ctx)
}
