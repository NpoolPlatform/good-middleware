...
func (*AppGoodBase) scanValues(columns []string) ([]interface{}, error) {
	...
	for i := range columns {
		...
		default:
			return nil, fmt.Errorf("unexpected column %q for type AppGoodBase", columns[i])
		}
	}
	return values, nil
}
...
func (agb *AppGoodBase) assignValues(columns []string, values []interface{}) error {
...
func (agb *AppGoodBase) Update() *AppGoodBaseUpdateOne {
	return (&AppGoodBaseClient{config: agb.config}).UpdateOne(agb)
}
...
func (agb *AppGoodBase) Unwrap() *AppGoodBase {
	...
	if !ok {
		panic("ent: AppGoodBase is not a transactional entity")
	}
...
func (agb AppGoodBases) config(cfg config) {
	for _i := range agb {
		agb[_i].config = cfg
	}
}
