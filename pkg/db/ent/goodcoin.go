...
func (*GoodCoin) scanValues(columns []string) ([]interface{}, error) {
	...
	for i := range columns {
		...
		default:
			return nil, fmt.Errorf("unexpected column %q for type GoodCoin", columns[i])
		}
	}
	return values, nil
}
...
func (gc *GoodCoin) assignValues(columns []string, values []interface{}) error {
...
func (gc *GoodCoin) Update() *GoodCoinUpdateOne {
	return (&GoodCoinClient{config: gc.config}).UpdateOne(gc)
}
...
func (gc *GoodCoin) Unwrap() *GoodCoin {
	...
	if !ok {
		panic("ent: GoodCoin is not a transactional entity")
	}
...
func (gc GoodCoins) config(cfg config) {
	for _i := range gc {
		gc[_i].config = cfg
	}
}
