...
func (aglu *AppGoodLabelUpdate) Save(ctx context.Context) (int, error) {
	...
	if err := aglu.defaults(); err != nil {
		return 0, err
	}
	if len(aglu.hooks) == 0 {
		affected, err = aglu.sqlSave(ctx)
	} else {
		var mut Mutator = MutateFunc(func(ctx context.Context, m Mutation) (Value, error) {
			...
			aglu.mutation = mutation
			affected, err = aglu.sqlSave(ctx)
			mutation.done = true
			return affected, err
		})
		for i := len(aglu.hooks) - 1; i >= 0; i-- {
			if aglu.hooks[i] == nil {
				return 0, fmt.Errorf("ent: uninitialized hook (forgotten import ent/runtime?)")
			}
			mut = aglu.hooks[i](mut)
		}
		if _, err := mut.Mutate(ctx, aglu.mutation); err != nil {
			return 0, err
		}
	}
	return affected, err
...
func (aglu *AppGoodLabelUpdate) defaults() error {
	if _, ok := aglu.mutation.UpdatedAt(); !ok {
		if appgoodlabel.UpdateDefaultUpdatedAt == nil {
			return fmt.Errorf("ent: uninitialized appgoodlabel.UpdateDefaultUpdatedAt (forgotten import ent/runtime?)")
		}
...
func (aglu *AppGoodLabelUpdate) sqlSave(ctx context.Context) (n int, err error) {
	...
	_spec.Modifiers = aglu.modifiers
...
func (agluo *AppGoodLabelUpdateOne) Save(ctx context.Context) (*AppGoodLabel, error) {
	...
	if err := agluo.defaults(); err != nil {
		return nil, err
	}
	if len(agluo.hooks) == 0 {
		node, err = agluo.sqlSave(ctx)
	} else {
		var mut Mutator = MutateFunc(func(ctx context.Context, m Mutation) (Value, error) {
			mutation, ok := m.(*AppGoodLabelMutation)
			if !ok {
			...
			agluo.mutation = mutation
			node, err = agluo.sqlSave(ctx)
			mutation.done = true
			return node, err
		})
		for i := len(agluo.hooks) - 1; i >= 0; i-- {
			if agluo.hooks[i] == nil {
				return nil, fmt.Errorf("ent: uninitialized hook (forgotten import ent/runtime?)")
			}
			mut = agluo.hooks[i](mut)
		}
		v, err := mut.Mutate(ctx, agluo.mutation)
		if err != nil {
			return nil, err
		}
...
// defaults sets the default values of the builder before save.
func (agluo *AppGoodLabelUpdateOne) defaults() error {
...
func (agluo *AppGoodLabelUpdateOne) sqlSave(ctx context.Context) (_node *AppGoodLabel, err error) {
	...
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`ent: missing "AppGoodLabel.id" for update`)}
	}
	...
	_spec.Modifiers = agluo.modifiers
