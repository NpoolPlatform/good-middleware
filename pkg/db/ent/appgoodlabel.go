...
func (*AppGoodLabel) scanValues(columns []string) ([]interface{}, error) {
	...
	for i := range columns {
		...
		default:
			return nil, fmt.Errorf("unexpected column %q for type AppGoodLabel", columns[i])
		}
	}
	return values, nil
}
...
func (agl *AppGoodLabel) assignValues(columns []string, values []interface{}) error {
...
func (agl *AppGoodLabel) Update() *AppGoodLabelUpdateOne {
	return (&AppGoodLabelClient{config: agl.config}).UpdateOne(agl)
}
...
func (agl *AppGoodLabel) Unwrap() *AppGoodLabel {
	...
	if !ok {
		panic("ent: AppGoodLabel is not a transactional entity")
	}
...
func (agl AppGoodLabels) config(cfg config) {
	for _i := range agl {
		agl[_i].config = cfg
	}
}
