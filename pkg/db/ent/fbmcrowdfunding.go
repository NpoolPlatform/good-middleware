...
func (*FbmCrowdFunding) scanValues(columns []string) ([]interface{}, error) {
	...
	for i := range columns {
		...
		default:
			return nil, fmt.Errorf("unexpected column %q for type FbmCrowdFunding", columns[i])
		}
	}
	return values, nil
}
...
func (fcf *FbmCrowdFunding) assignValues(columns []string, values []interface{}) error {
...
func (fcf *FbmCrowdFunding) Update() *FbmCrowdFundingUpdateOne {
	return (&FbmCrowdFundingClient{config: fcf.config}).UpdateOne(fcf)
}
...
func (fcf *FbmCrowdFunding) Unwrap() *FbmCrowdFunding {
	...
	if !ok {
		panic("ent: FbmCrowdFunding is not a transactional entity")
	}
...
func (fcf FbmCrowdFundings) config(cfg config) {
	for _i := range fcf {
		fcf[_i].config = cfg
	}
}
