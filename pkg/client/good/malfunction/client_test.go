...
func createMalfunction(t *testing.T) {
	err := CreateMalfunction(context.Background(), &npool.MalfunctionReq{
		EntID:             &ret.EntID,
		GoodID:            &ret.GoodID,
		Title:             &ret.Title,
		Message:           &ret.Message,
		...
		CompensateSeconds: &ret.CompensateSeconds,
	})
	if assert.Nil(t, err) {
		info, err := GetMalfunction(context.Background(), ret.EntID)
		if assert.Nil(t, err) {
...
func updateMalfunction(t *testing.T) {
	ret.StartAt += 1
	err := UpdateMalfunction(context.Background(), &npool.MalfunctionReq{
		ID:                &ret.ID,
		EntID:             &ret.EntID,
		Title:             &ret.Title,
		Message:           &ret.Message,
...
func getMalfunctions(t *testing.T) {
	infos, total, err := GetMalfunctions(context.Background(), &npool.Conds{
...
func deleteMalfunction(t *testing.T) {
	err := DeleteMalfunction(context.Background(), &ret.ID, &ret.EntID)
	assert.Nil(t, err)

	info, err := GetMalfunctionOnly(context.Background(), &npool.Conds{
		ID:     &basetypes.Uint32Val{Op: cruder.EQ, Value: ret.ID},
		EntID:  &basetypes.StringVal{Op: cruder.EQ, Value: ret.EntID},
		EntIDs: &basetypes.StringSliceVal{Op: cruder.IN, Value: []string{ret.EntID}},
		GoodID: &basetypes.StringVal{Op: cruder.EQ, Value: ret.GoodID},
