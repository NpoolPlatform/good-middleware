...
func createDisplayName(t *testing.T) {
	err := CreateDisplayName(context.Background(), &npool.DisplayNameReq{
		EntID:     &ret.EntID,
		AppGoodID: &ret.AppGoodID,
		Name:      &ret.Name,
		Index:     &ret.Index,
	...
	if assert.Nil(t, err) {
		info, err := GetDisplayName(context.Background(), ret.EntID)
		if assert.Nil(t, err) {
...
func updateDisplayName(t *testing.T) {
	err := UpdateDisplayName(context.Background(), &npool.DisplayNameReq{
		ID:   &ret.ID,
		Name: &ret.Name,
	})
	if assert.Nil(t, err) {
...
//nolint
func getDisplayNameOnly(t *testing.T) {
	info, err := GetDisplayNameOnly(context.Background(), &npool.Conds{
		ID:         &basetypes.Uint32Val{Op: cruder.EQ, Value: ret.ID},
		EntID:      &basetypes.StringVal{Op: cruder.EQ, Value: ret.EntID},
		AppID:      &basetypes.StringVal{Op: cruder.EQ, Value: ret.AppID},
		AppGoodID:  &basetypes.StringVal{Op: cruder.EQ, Value: ret.AppGoodID},
...
func deleteDisplayName(t *testing.T) {
	err := DeleteDisplayName(context.Background(), &ret.ID, &ret.EntID)
	assert.Nil(t, err)
...
func TestDisplayName(t *testing.T) {
	...
	t.Run("getDisplayNameOnly", getDisplayNameOnly)
