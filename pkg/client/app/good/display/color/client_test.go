...
func createDisplayColor(t *testing.T) {
	err := CreateDisplayColor(context.Background(), &npool.DisplayColorReq{
		EntID:     &ret.EntID,
		AppGoodID: &ret.AppGoodID,
		Color:     &ret.Color,
		Index:     &ret.Index,
	...
	if assert.Nil(t, err) {
		info, err := GetDisplayColor(context.Background(), ret.EntID)
		if assert.Nil(t, err) {
...
func updateDisplayColor(t *testing.T) {
	err := UpdateDisplayColor(context.Background(), &npool.DisplayColorReq{
		ID:    &ret.ID,
		Color: &ret.Color,
	})
	if assert.Nil(t, err) {
...
//nolint
func getDisplayColorOnly(t *testing.T) {
	info, err := GetDisplayColorOnly(context.Background(), &npool.Conds{
		ID:         &basetypes.Uint32Val{Op: cruder.EQ, Value: ret.ID},
		EntID:      &basetypes.StringVal{Op: cruder.EQ, Value: ret.EntID},
		AppID:      &basetypes.StringVal{Op: cruder.EQ, Value: ret.AppID},
		AppGoodID:  &basetypes.StringVal{Op: cruder.EQ, Value: ret.AppGoodID},
...
func deleteDisplayColor(t *testing.T) {
	err := DeleteDisplayColor(context.Background(), &ret.ID, &ret.EntID)
	assert.Nil(t, err)
...
func TestDisplayColor(t *testing.T) {
	...
	t.Run("getDisplayColorOnly", getDisplayColorOnly)
