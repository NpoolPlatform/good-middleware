...
func createScore(t *testing.T) {
	err := CreateScore(context.Background(), &npool.ScoreReq{
		EntID:     &ret.EntID,
		UserID:    &ret.UserID,
		AppGoodID: &ret.AppGoodID,
		Score:     &ret.Score,
	...
	if assert.Nil(t, err) {
		info, err := GetScore(context.Background(), ret.EntID)
		if assert.Nil(t, err) {
...
func updateScore(t *testing.T) {
	err := UpdateScore(context.Background(), &npool.ScoreReq{
		ID:    &ret.ID,
		Score: &ret.Score,
	})
	if assert.Nil(t, err) {
...
func deleteScore(t *testing.T) {
	err := DeleteScore(context.Background(), &ret.ID, &ret.EntID)
	assert.Nil(t, err)
...
func TestScore(t *testing.T) {
	...
	t.Run("updateScore", updateScore)
	t.Run("getScores", getScores)
	t.Run("getScoreOnly", getScoreOnly)
