...
func (h *updateHandler) constructPledgeSQL() {
	set := "set "
...
func (h *updateHandler) updatePledge(ctx context.Context, tx *ent.Tx) error {
	if h.sqlPledge == "" {
		return nil
	}
	n, err := h.execSQL(ctx, tx, h.sqlPledge)
	if err != nil || n != 1 {
		return wlog.Errorf("fail update pledge: %v", err)
	}
	return nil
}
...
func (h *Handler) UpdatePledge(ctx context.Context) error {
	...
	if h.GoodID == nil {
		h.GoodID = &handler.pledge.GoodID
	...
	return db.WithTx(ctx, func(_ctx context.Context, tx *ent.Tx) error {
		...
		if err := handler.updatePledge(_ctx, tx); err != nil {
			return wlog.WrapError(err)
		}
		return nil
	})
